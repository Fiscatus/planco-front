...
export const ProcessFormComponent = ({ label, description, context, enabled = true, readOnly = false }: ProcessFormComponentProps) => {
  const { data: formData, isLoading } = useForm(context, enabled);
  ...
  const fields: FormField[] = formData?.config?.fields || [];
  ...
  const handleSave = () => {
    ...
    submitMutation.mutate({
      ...
      onError: (error: any) => {
        const message = error?.response?.data?.message || "Erro ao salvar formulário";
                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      "&:hover": { borderColor: readOnly ? "#E4E6EB" : "#1877F2", bgcolor: checked ? "#E7F3FF" : readOnly ? "#fff" : "#F5FAFF" },
                    }}
                  >
  ...
  const headerContent = (onClose?: () => void) => (
    <Box sx={{ px: 3, py: 2, bgcolor: "#FAFBFC", borderBottom: "1px solid #E4E6EB", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        ...
        <Typography sx={{ fontWeight: 700, color: "#0f172a", fontSize: onClose ? "1.125rem" : "1rem" }}>
          {label || "Formulário"}
        </Typography>
        {description && (
  ...
  return (
    <>
      ...
      <Dialog open={showConfirmDialog} onClose={() => setShowConfirmDialog(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ color: "#64748b" }}>Deseja salvar as alterações do formulário?</Typography>
        </DialogContent>
