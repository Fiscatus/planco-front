...
const NotFoundPage = () => {
  ...
  const hasOrganization = user && user.org;
  const isLoggedInWithoutOrg = user && !user.org;
  const isNotLoggedIn = !user;

  const shouldUsePublicLayout = isNotLoggedIn || isLoggedInWithoutOrg;

  const handleGoHome = () => {
    if (hasOrganization) {
      navigate("/organization-home");
    } else if (isLoggedInWithoutOrg) {
      navigate("/invites");
    } else {
      navigate("/auth");
    }
        ...
        >
          <Box
            ...
            sx={{
              ...
              opacity: 0.3,
              background:
                "radial-gradient(60% 60% at 50% 40%, rgb(137, 78, 238) 0%, transparent 70%)",
            }}
          />
