import { Add as AddIcon, Edit as EditIcon, GroupAdd as GroupAddIcon, Refresh as RefreshIcon, Search as SearchIcon } from '@mui/icons-material';
import {
  Alert,
  Box,
  Button,
...
const GerenciaSection = () => {
  ...
  }, [selectedDept, selectedUserIds, addMembersBulk, showNotification, fetchDepartments, pagination.page, pagination.limit, search, fetchUsers]);
  ...
  const searchResponsavel = useCallback(async (query: string) => {
  ...
  const handleOpenEdit = useCallback(async (dept: Department) => {
    setDepartmentForm({
  ...
  }, [departmentForm, editDialogOpen, selectedDept, updateDepartment, createDepartment, showNotification, handleCloseDialogs, fetchDepartments, pagination.page, pagination.limit, search]);
  ...
  }, [departmentToDelete, deleteDepartment, showNotification, handleCloseDialogs, fetchDepartments, pagination.page, pagination.limit, search]);
  ...
  const handleUserPageChange = useCallback((_event: unknown, newPage: number) => {
            ...
            <CardHeader
              ...
              action={
                <Stack direction='row' spacing={1}>
                  ...
                  <Button startIcon={<AddIcon />} variant='contained' size='small' sx={{ textTransform: 'none' }} onClick={handleOpenCreate}>Nova Gerência</Button>
                </Stack>
              }
            />
            <CardContent>
  ...
  return (
                        <ListItem key={dept._id} disableGutters sx={{ mb: 1 }}>
                            ...
                            <ListItemText
                              primary={
                                <Stack direction='row' spacing={1} alignItems='center'>
                                  ...
                                  <Chip size='small' label={dept.department_acronym} variant='outlined' />
                                </Stack>
                              }
                            />
              ...
              <CardHeader
                title={
                  <Stack direction='row' spacing={1} alignItems='center'>
                    ...
                    {selected && <Chip size='small' label={selected.department_acronym} variant='outlined' />}
                  </Stack>
                }
                subheader={selected ? 'Gerência selecionada' : 'Selecione uma gerência à esquerda'}
                action={
                  <Stack direction='row' spacing={1}>
                    <Button variant='outlined' size='small' startIcon={<EditIcon />} onClick={() => selected && handleOpenEdit(selected)}>Editar</Button>
                    <Button 
                      variant='outlined' 
                      size='small' 
                      color='error' 
              ...
              <CardContent>
                {selected ? (
                  <Grid container spacing={2}>
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                      <Typography variant='subtitle2' color='text.secondary'>Responsável gerência</Typography>
  ...
  return (
    ...
    <Dialog open={createDialogOpen || editDialogOpen} onClose={handleCloseDialogs} fullWidth maxWidth='md'>
            ...
            <Grid container spacing={3}>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  onChange={(e) => setDepartmentForm(prev => ({ ...prev, department_acronym: e.target.value.toUpperCase() }))}
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  ...
                  onChange={(e) => setDepartmentForm(prev => ({ ...prev, deparment_email: e.target.value.toLowerCase() }))}
                  required
                  variant='outlined'
                />
              </Grid>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <FormControl fullWidth variant='outlined'>
                  ...
                  <Select
                    value={responsavelUsers.find(u => u._id === departmentForm.responsavelUserId) ? departmentForm.responsavelUserId || '' : ''}
                    label='Responsável'
                    onChange={(e) => {
      ...
      <DialogActions>
        <Button onClick={handleCloseDialogs} variant='outlined'>Cancelar</Button>
        <Button onClick={handleSaveDepartment} variant='contained' disabled={savingDepartment}>
          {savingDepartment ? 'Salvando...' : (editDialogOpen ? 'Atualizar' : 'Criar')}
        </Button>
      </DialogActions>
    </Dialog>

    <Dialog open={deleteDialogOpen} onClose={handleCloseDialogs} fullWidth maxWidth='sm'>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          {departmentToDelete && (
            <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
              ...
              <Typography variant='body2'><strong>Nome:</strong> {departmentToDelete.department_name}</Typography>
              <Typography variant='body2'><strong>Sigla:</strong> {departmentToDelete.department_acronym}</Typography>
              <Typography variant='body2'><strong>E-mail:</strong> {departmentToDelete.deparment_email}</Typography>
            </Box>
          )}
    ...
    >
                  ...
                  paginatedUsers.map((u) => (
                    <TableRow key={u._id}>
                      ...
                      <TableCell align='center'>
                        ...
                        >
                          {(u as any).isMember ? 'Já é membro' : selectedUserIds.includes(u._id || '') ? 'Remover' : 'Adicionar'}
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))
      ...
      <DialogActions>
        ...
        <Button onClick={handleSaveMembers} variant='contained' disabled={savingMembers || !selectedDept}>
